  ...
  useEffect(() => {
    if (school && open) {
      setFormData({
        ...
        provinceId: school?.provinceId || 0,
        regencyId: school?.regencyId || 0,
        districtId: school?.districtId || 0,
      });
      if (school.provinceId && school.regencyId) {
        loadInitialRegions(school.provinceId, school.regencyId);
      }
    }
  }, [school, open]);

  const loadInitialRegions = async (pId: number, rId: number) => {
    setFetchingRegions(true);
    try {
      const [regData, distData] = await Promise.all([
        SchoolService.getRegencies(pId),
        SchoolService.getDistricts(rId),
      ]);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-135 overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Edit Detail Sekolah</SheetTitle>
          <SheetDescription>
            Perbarui informasi sekolah dan lokasi administratifnya.
          </SheetDescription>
        </SheetHeader>
